      ...
      <div className="card p-6">
        ...
        {recentJobs.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500 dark:text-gray-400 transition-colors">
            No jobs found. Start by commenting on an issue or PR with a command like <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded text-sm">/refactor</code>
          </div>
        )}
      </div>
    </div>
  ...
  <div className="card p-6">
    <div className="flex items-center">
      ...
      <div className="ml-4">
        ...
        <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 transition-colors">{value}</p>
      </div>
    </div>
  </div>
)
...
const JobCard = ({ job }: { job: Job }) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'queued': return 'text-yellow-600 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30'
