...
export interface IssueAnalysis {
  taskType: 'bug_fix' | 'refactor' | 'test_generation' | 'documentation' | 'security_audit' | 'code_quality'
...
export class IssueAnalyzerService {
  ...
  private determineTaskType(
    ...
    const bugKeywords = ['bug', 'error', 'issue', 'problem', 'broken', 'fail', 'crash', 'exception', 'not working']
    const refactorKeywords = ['refactor', 'improve', 'optimize', 'enhance', 'feature', 'add', 'implement']
    const testKeywords = ['test', 'testing', 'coverage', 'spec', 'unit test', 'integration test']
    const docKeywords = ['documentation', 'docs', 'readme', 'guide', 'comment', 'document']
    const securityKeywords = ['security', 'vulnerability', 'auth', 'permission', 'xss', 'sql injection']
    const qualityKeywords = ['quality', 'lint', 'format', 'style', 'code quality']
  ...
  private estimateComplexity(content: string): IssueAnalysis['complexity'] {
    const complexityIndicators = {
      high: ['multiple files', 'architecture', 'database', 'breaking change', 'major refactor'],
  ...
  private determinePriority(
    ...
    const combined = `${title} ${body}`
    const criticalKeywords = ['critical', 'urgent', 'production', 'security', 'data loss']
    const highKeywords = ['important', 'blocker', 'broken', 'failing']
    const lowKeywords = ['enhancement', 'nice to have', 'future', 'improvement']
  ...
  private extractKeywords(content: string): string[] {
    const allKeywords = [
      'bug', 'error', 'issue', 'problem', 'broken', 'fail', 'crash',
      'refactor', 'improve', 'optimize', 'enhance', 'feature',
      'test', 'testing', 'coverage', 'documentation', 'security'
    ]
  ...
  private estimateEffort(complexity: string, taskType: string): number {
    ...
    return baseEffort[taskType as keyof typeof baseEffort] * 
  ...
  private calculateConfidence(keywords: string[], taskType: string): number {
    ...
    const relevant = relevantKeywords[taskType as keyof typeof relevantKeywords] || []
  ...
  shouldProcessIssue(issueEvent: IssueEvent): boolean {
    ...
    if (labelNames.includes('no-automation') || labelNames.includes('wontfix')) {
      return false
    }
