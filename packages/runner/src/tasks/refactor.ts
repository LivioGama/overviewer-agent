...
export class RefactorTask extends BaseTask {
  async execute(job: Job): Promise<TaskResult> {
    const instructions = job.taskParams.args || 'Improve code quality and maintainability'
    ...
    const prBody = this.generatePullRequestBody(instructions, modifiedFiles, totalChanges)
