...
export class BugFixTask extends BaseTask {
  async execute(job: Job): Promise<TaskResult> {
    ...
    if (job.taskParams.autoTriggered) {
      ...
      await this.updateIssueProgress(job, 'analyzing', 'Analyzing the issue content and determining the best approach...')
    }

    const branchName = await this.createWorkingBranch(job, 'bugfix/')
    const errorDescription = job.taskParams.issueBody || job.taskParams.args || 'General bug fixes'
    
    await this.updateIssueProgress(job, 'fixing', 'Scanning codebase and implementing fixes...')
    ...
    await this.updateIssueProgress(job, 'creating_pr', 'Creating pull request with the proposed changes...')
    ...
    if (job.taskParams.autoTriggered && pullRequestUrl) {
      console.log(`🤖 Self-review would be triggered for PR: ${pullRequestUrl}`)
  ...
  private generateRooCodePullRequestBody(job: Job, fixedFiles: string[]): string {
