...
export class TestGenerationTask extends BaseTask {
  ...
  private async generateTestForFile(sourceFile: string, job: Job): Promise<string | null> {
  ...
  private async findExistingTestFile(sourceFile: string): Promise<string | null> {
  ...
  private detectTestFramework(): string {
    try {
      ...
      if (packageJson.devDependencies) {
        ...
        if (packageJson.devDependencies['@testing-library/react']) return 'react-testing-library'
      }
    } catch (error) {
