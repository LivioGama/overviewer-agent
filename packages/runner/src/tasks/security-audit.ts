...
export class SecurityAuditTask extends BaseTask {
  async execute(job: Job): Promise<TaskResult> {
    ...
    const pullRequestUrl = fixedFiles.length > 0 ? 
  ...
  private async auditFile(filePath: string, job: Job): Promise<{
    vulnerabilities: any[]
    fixedContent?: string
  }> {
...
`
    ...
    return this.ollama.generate({
      ...
      prompt,
      system: 'You are a security expert. Fix vulnerabilities while maintaining functionality.'
    })
  }
...
*Report generated by Ollama Turbo Agent*`
  ...
  private generatePullRequestBody(findings: any[], fixedFiles: string[]): string {
