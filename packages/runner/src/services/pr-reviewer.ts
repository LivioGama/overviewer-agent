...
export class PRReviewerService {
  ...
  private async analyzeChanges(
    ...
    files: Array<{ filename: string; additions: number; deletions: number; patch?: string; status: string }>,
    ...
    const analysisPrompt = this.buildAnalysisPrompt(prData, changes, originalIssue)
    
    try {
      const analysis = await this.ollama.generate({
        model: job?.taskParams.model || 'gpt-oss:120b',
        prompt: analysisPrompt,
        system: 'You are an expert code reviewer. Analyze the changes and provide structured feedback in JSON format.'
      })
  ...
  private buildAnalysisPrompt(
    prData: { title: string; body: string },
    changes: Array<{ filename: string; additions: number; deletions: number; patch?: string }>,
    originalIssue?: { title: string; body: string }
  ): string {
...
Respond in JSON format:
{
  ...
  private parseAnalysisResult(analysis: string, changes: Array<{ filename: string; additions: number; deletions: number; patch?: string }>): ReviewResult {
    try {
      const parsed = JSON.parse(analysis)
      
      const comments: ReviewComment[] = (parsed.comments || []).map((comment: { file: string; line?: number; message: string; severity?: string }) => ({
        path: comment.file,
        line: comment.line || 1,
  ...
  private basicAnalysis(changes: Array<{ additions: number; deletions: number; filename: string }>, originalIssue?: { title: string; body: string }): ReviewResult {
    // Basic heuristic analysis
    const totalChanges = changes.reduce((sum, file) => sum + file.additions + file.deletions, 0)
  ...
  private calculateConfidence(parsed: { addresses_issue?: boolean; minimal_changes?: boolean; code_quality?: number; risks?: string[] }): number {
  ...
  private generateSuggestions(parsed: { minimal_changes?: boolean; code_quality?: number; risks?: string[] }, changes: Array<{ filename: string }>): string[] {
    const suggestions = []

    if (!parsed.minimal_changes) {
      suggestions.push('Consider breaking down changes into smaller, more focused commits')
    }
    ...
    const hasTests = changes.some(file => 
  ...
  private async postReviewComments(
    ...
    for (const comment of comments) {
      try {
        ...
        console.warn(`Failed to post review comment on ${comment.path}:${comment.line}:`, error)
      }
    }
  }
...
export const createPRReviewerService = (ollama: OllamaService, octokit: Octokit) => 
